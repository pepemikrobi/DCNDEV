...
def push_config(host, username, password, config_lines):
    """Send config lines to an IOS device via SSH and return the session output."""
    import paramiko
    ...
    output = []
    output.append(send("terminal length 0"))
    output.append(send("configure terminal"))
    for line in config_lines:
        output.append(send(line))
    output.append(send("end"))
    output.append(send("write memory"))
    time.sleep(2)
    output.append(shell.recv(65535).decode(errors="replace"))
